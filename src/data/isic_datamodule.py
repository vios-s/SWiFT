...
class ISICDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if not self.data_train and not self.data_val and not self.data_test:
            self.data_train = ISICDataset(self.hparams.data_dir + self.hparams.train_dataset, transform=self.transform)
            self.data_val = ISICDataset(self.hparams.data_dir + self.hparams.val_dataset, transform=self.transform)
            self.data_test = ISICDataset(self.hparams.data_dir + self.hparams.test_dataset, transform=self.test_transform)
            self.test = None

    # def get_weights(self):
    #     train_df = pd.read_csv(self.hparams.data_dir + self.hparams.dataset + '.csv',
    #                             low_memory=False).reset_index(drop=True)
    #     lst = train_df['targets'].value_counts().sort_index().tolist()
    #     sum_lst = sum(lst)
    #     class_freq = []
    #     for i in lst:
