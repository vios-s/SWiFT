...
class DenseNet(Extractor):
    def __init__(self, weights=True):
        ...
        self.enet = models.densenet121(weights=weights)
        self.dropout = nn.Dropout(p=0.5)
...
class ResNet34(Extractor):
    def __init__(self, weights=True):
        ...
        self.enet = models.resnet34(weights=weights)
        self.dropout = nn.Dropout(p=0.5)
        in_ch = self.enet.fc.in_features
        self.enet.fc = nn.Identity()
        
class Swin_v2_S(Extractor):
    def __init__(self, weights=True):
        """
        Swin V2 B feature extractor
        """
        super(Swin_v2_S, self).__init__()

        self.enet = models.swin_v2_s(weights=weights)
        self.dropout = nn.Dropout(p=0.5)
        in_ch = self.enet.head.in_features
        self.enet.head = nn.Identity()
        
class MaxVit(Extractor):
    def __init__(self, weights=True):
        """
        ...
        self.enet = models.vit_b_16(weights=weights)
        self.dropout = nn.Dropout(p=0.5)
        in_ch = self.enet.head.in_features
        self.enet.head = nn.Identity()