...
def dataset_balance(df, attribute, target, downsample=0):
    ...
    if len(group0) > len(group1):
        ...
        # index = np.random.randint(len(group0), size=len(group1))
        # group0 = group0.iloc[list(index)]
    else:
        df0, df1 = split_dataset(group1, group0, target, downsample)
        # index = np.random.randint(len(group1), size=len(group0))
        # group1 = group1.iloc[list(index)]
    csv = pd.concat([df0, df1], ignore_index=True)
    # csv = pd.concat([group0, group1], ignore_index=True)
    return csv
...
def split_dataset(df0, df1, target, downsample):
    ...
    if downsample:
        ...
        up_target1 = group1_target1.iloc[list(index_target1)]
